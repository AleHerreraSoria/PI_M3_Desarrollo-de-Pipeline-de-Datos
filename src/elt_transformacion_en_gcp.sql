-- #############################################################################
-- AVANCE 3: SCRIPT DE TRANSFORMACIÓN DE DATOS (ELT)
-- #############################################################################

-- =============================================================================
-- PASO 1: CAPA TRANSFORMED (PLATA) - Limpieza y Enriquecimiento
-- =============================================================================

-- 1.1. Crea la tabla principal de listados limpios y enriquecidos.
CREATE OR REPLACE TABLE `eng-name-468100-g3.transformed_data.listings_cleaned` AS
WITH Staging_Listings AS (
  -- Selecciona y castea los tipos de datos de la tabla cruda
  SELECT
    id AS listing_id,
    name,
    host_id,
    ...
    COALESCE(SAFE_CAST(reviews_per_month AS FLOAT64), 0) AS reviews_per_month, -- Convierte nulos a 0
  ...
  WHERE
    price > 0 -- Excluye datos considerados inválidos
),

-- 1.2. Obtiene la última cotización del dólar disponible.
Latest_Exchange_Rate AS (
  SELECT
    valor_venta
  FROM
    `eng-name-468100-g3.raw_data.bcra_exchange_rates`
  -- Se asegura de tomar el valor más reciente en caso de haber múltiples registros
  QUALIFY ROW_NUMBER() OVER(ORDER BY fecha DESC) = 1
)

-- 1.3. Une los datos de listados con la tasa de cambio para crear la tabla final.
SELECT
  listings.*,
  exchange.valor_venta AS usd_to_ars_rate,
  (listings.price_usd * exchange.valor_venta) AS price_ars -- Calcula el precio en ARS
FROM
  Staging_Listings AS listings
CROSS JOIN -- Se usa CROSS JOIN porque solo hay una fila en Latest_Exchange_Rate
  Latest_Exchange_Rate AS exchange;


-- =============================================================================
-- PASO 2: CAPA DE NEGOCIO (ORO) - Tablas Agregadas para Análisis
-- =============================================================================

-- 2.1. PREGUNTA: ¿Cuál es el precio promedio por barrio y distrito?
...
-- 2.2. PREGUNTA: ¿Qué tipo de habitación es el más ofrecido y genera más revenue?
...
-- 2.3. PREGUNTA: ¿Cuáles son los anfitriones con más propiedades?
...
-- 2.4. PREGUNTA: ¿Evolución de reseñas por mes y distrito?
